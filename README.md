...
3. 填入書籍編號後按「開始下載」。「停止下載」只中止目前的下載，介面可以繼續使用；按「結束程式」或關閉分頁則會結束程式。
...
執行中會顯示已抓取章數、進度條與預估剩餘時間。預估值以目錄章數和已經花掉的時間推算，只涵蓋章節抓取，不含圖片下載與轉檔。

介面只監聽 `127.0.0.1`，而且每次啟動都會產生一組隨機權杖，避免其他網頁或程式操控爬蟲。關閉分頁時網頁會通知程式，程式約 3 秒後結束，不會留下背景程序；重新整理不算關閉。Chrome 可能把背景分頁捨棄，這不會中斷下載，點回分頁即可繼續看進度；沒有下載進行且頁面超過 60 秒沒回報時程式才會自行結束。想避免分頁被捨棄，可把分頁釘選，或在 Chrome「效能」設定把 `127.0.0.1` 加入「永遠保持啟用」。
...
| 情況 | 目前行為 |
|---|---|
| 單次 HTTP 請求失敗 | 自動重試最多 10 次，間隔指數成長並以 10 秒為上限 |
| 用瀏覽器取頁失敗（例如 `PageDisconnectedError`） | 同樣重試最多 10 次，過程寫在執行紀錄裡 |
| 某一頁反覆取不到 | 外層迴圈沒有放棄條件，會一直重試下去。請按「停止下載」中止 |
| 目錄頁抓不到 | 整次執行失敗，網頁會顯示錯誤訊息 |
| 個別圖片 404 | 略過該圖後繼續 |
| 字型混淆辨識失敗 | 記錄錯誤後略過，該段文字維持原樣 |

續傳的範圍要說清楚。這個函式庫只在**整本書的文字全部抓完之後**，才把資料寫進 `pickle/` 一次，中途沒有任何檢查點：

- 在抓文字的階段關掉程式，該次進度會全部遺失，下次得從頭開始。
- 文字抓完之後，在下載圖片或產生 EPUB 的階段中斷，下次啟動時勾選「沿用上次未完成的進度」就能接續。

「沿用上次未完成的進度」對應的是函式庫原本的終端機提問。網頁介面背後沒有終端機，直接執行那段提問會拋出 `EOFError`，所以改由這個勾選回答。
